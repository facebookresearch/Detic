...
action2label = re.findall('\s*;\s+\d+\s(.*)\n\s+(?:\s*;.*\n\s+)?\(:action ([\w-]+)', open(GPA_PDDL).read())
action2label = {k:v for v, k in action2label}
...
for f in ['VAL','TRAIN']:
    ...
    for video in tqdm.tqdm(SE_ann.keys()):
        ...
        if void_video:
            #print('handling void video. Action is:',video_action)
        ...
        for frame in SE_ann[video]:
            ...
            for obj in frame_objects:
                ...
                # check if the obj category is in SSData
                if obj['category'] not in SS_CLASSES:
                    SS_CLASSES[obj['category']] = obj_id
                    obj_id += 1

                if obj['category'] not in OBJECT_CATEGORIES:
                    OBJECT_CATEGORIES[obj['category']] = {}
                    OBJECT_CATEGORIES[obj['category']]['name'] = obj['category']
                    OBJECT_CATEGORIES[obj['category']]['instance_count'] = 1
                    OBJECT_CATEGORIES[obj['category']]['id'] = SS_CLASSES[obj['category']]
                    OBJECT_CATEGORIES[obj['category']]['supercategory'] = 'OBJECT'
                    SSdata['categories'].append({
                        'id':OBJECT_CATEGORIES[obj['category']]['id'], 
                        'name':OBJECT_CATEGORIES[obj['category']]['name'], 
                        'supercategory':'OBJECT'})
                    #print('new object category added to SSdata:',SSdata['categories'][-1],video)
                    #print(obj['category'])
                    #print(ss_plhs)
                    #print(frame['gt_placeholders'])
                    #input()
                else:
                    OBJECT_CATEGORIES[obj['category']]['instance_count'] += 1

                # initialize object annotation and add to SSdata
                obj_dict = {}
                obj_dict['id'] = ann_id
                obj_dict['image_id'] = image_id
                obj_dict['category_id'] = OBJECT_CATEGORIES[obj['category']]['id']
                ...
                if not void_video: 
                    ...
                    obj_precs = [p for p in obj_precs if '(not (=' not in p]
                    obj_precs = [p for p in obj_precs if 'visible' not in p]

                    # filter which conditions apply to the object
                    if obj_st_cat == 'hand':
                        precs = []
                        for p in obj_precs:
                            if '(in ' in p and 'hand' in p:
                                p = p.replace('hand','').replace('in ','holding ')
                            elif 'hand' in p:
                                p = p.replace('hand','')
                            else:
                                continue
                        ...
                        for p in obj_precs:
                            ...
                            precs.append(p)
                        obj_precs = precs
                    obj_precs = [p.replace(' )',')') for p in obj_precs]
                    for iobj in range(npddl_objects):
                        obj_precs = [p.replace(GPA_int2alpha[iobj],ss_plhs[iobj]) for p in obj_precs]
                    ...
                    for p in obj_precs:
                        ...
                        if p not in OBJECT_PREDICATES:
                            ...
                            OBJECT_PREDICATES[p]['supercategory'] = 'OBJECT'
